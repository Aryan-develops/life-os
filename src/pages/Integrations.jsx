...
export default function Integrations() {
  ...
  const cronometerSteps = [
    'Open Cronometer on the web (cronometer.com)',
    'Go to Profile → Export Data',
    'Select date range and export "Servings" as CSV',
    'Go to the Nutrition page in Life OS',
    'Click "Cronometer CSV" button and select the downloaded file',
    'All daily macro totals will be imported automatically',
  ]
      ...
      <div className="card" style={{ padding: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: '#3d4560', marginTop: 2 }}>Import your food diary via CSV export</div>
          </div>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
          {cronometerSteps.map((step, i) => (
            <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
              <div style={{ width: 22, height: 22, borderRadius: '50%', flexShrink: 0, background: 'rgba(251,191,36,0.12)', border: '1px solid rgba(251,191,36,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: '#fbbf24', marginTop: 1 }}>{i + 1}</div>
              <div style={{ fontSize: 13, color: '#94a3b8', lineHeight: 1.5, paddingTop: 2 }}>{step}</div>
            </div>
          ))}
        </div>

        <div style={{ padding: '12px 14px', background: 'rgba(251,191,36,0.05)', borderRadius: 10, border: '1px solid rgba(251,191,36,0.12)', fontSize: 12, color: '#a8905a', lineHeight: 1.7 }}>
          Cronometer exports a <strong style={{ color: '#fbbf24' }}>servings.csv</strong> with per-food entries — Life OS automatically sums them into daily totals.
        </div>
      </div>
  ...
  "workouts": [{ "date": "2026-06-14", "type": "Strength", "duration_min": 60 }]
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
        {[
          { label: 'Cronometer Import', value: 'Ready', color: '#fbbf24' },
